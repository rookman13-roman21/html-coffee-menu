...
import { hasAccess, canDeleteWorkspaceData } from './auth.js';
...
export function filterSales(val) {
  ...
  const canDeleteMenuItems = canEditMenu && (canDeleteWorkspaceData() || !!(window.authorCanPublish && window.authorCanPublish()));
  ...
  const rows = filtered.map(d => {
    ...
    const actionBtn  = !canEditMenu ? '' : d.custom && canDeleteMenuItems
