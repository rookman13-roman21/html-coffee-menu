...
- Новый модуль `src/render/workspace.js`: компактный обзор, закреплённые ссылки, список заметок, список ссылок, Tiptap rich-text редактор заметки, активная toolbar-панель, undo/redo, task-list чекбоксы, изображения по URL, typography-автоформатирование, горячие клавиши, режим просмотра заметки перед редактированием, список файлов и предпросмотр изображений. Tiptap подключён lazy-load через dynamic import, чтобы основной bundle не разрастался при обычном входе в приложение.
