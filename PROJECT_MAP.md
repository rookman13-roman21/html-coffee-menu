...
- `Команда проекта`: добавлен workspace-слой для клиентов, которые запускают кофейню вместе с партнёрами/подрядчиками. В меню проекта доступны переключение workspace, invite-link, участники, append-only журнал ключевых действий и `Восстановление`. Пользователи с `access_drinks` / `access_finance` могут создавать свои проекты; приглашённые без оплаты работают как `guest-editor` только в чужом проекте. Удаление заведений, рецептов, поставщиков, сырья и полуфабрикатов в v1 доступно только владельцу; backend дополнительно блокирует destructive overwrite через `PUT /api/state` для editor/guest.
...
- Backend state-guard в `PUT /api/state`: для не-владельца сравнивается старый и новый workspace JSON; исчезновение локаций, `customDrinks`, `customMats`, `semiItems`, `suppliers` или `supplierBook` считается destructive overwrite и возвращает `403`.
