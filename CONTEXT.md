...
- Браузерные ключи localStorage остаются user-scoped: `mbs_locations__<user>` и `mbs_loc_<id>__<user>`; backend state теперь выбирается через активный `workspace_id`.
...
- Командный UI находится в меню проекта: список проектов, `Команда проекта`, `Журнал действий`, `Новый проект`.
...
- Журнал действий append-only и не должен превращаться в лог каждого autosave: фиксировать только осознанные действия пользователя.
...
- Backend state-guard: `PUT /api/state` для не-владельца сравнивает старый и новый JSON workspace и запрещает destructive overwrite, если исчезают локации, `customDrinks`, `customMats`, `semiItems`, `suppliers` или `supplierBook`. Обычные изменения полей, добавление новых сущностей и редактирование рецептур остаются разрешёнными. Это минимальный semantic diff, не полный аудит всех полей.
