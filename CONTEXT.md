...
- Данные v1 хранятся в `workspace.state` как `S.workspaceArea = { notes, links }`, без отдельных backend-таблиц.
...
- События логируются в журнал как `workspace_note_changed` и `workspace_link_changed`; в журнале есть фильтр `Рабочая зона`.
