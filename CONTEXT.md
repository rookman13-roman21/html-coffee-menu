...
- Журнал действий append-only и не должен превращаться в лог каждого autosave: фиксировать только осознанные действия пользователя. `project_opened` пишется не чаще одного раза на пользователя/проект за 30 минут; UI журнала поддерживает фильтры по разделам, роли участников и визуальную важность событий.
