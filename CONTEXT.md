...
## Сессия 68 — рабочая зона проекта

- Вкладка `Рабочая зона` добавлена первой в шапке перед `Бюджет`; route `/app/workspace`.
- Данные заметок и ссылок хранятся в `workspace.state` как `S.workspaceArea = { notes, links }`; приватные файлы заметок хранятся отдельно в `workspace_files` и `server/data/workspace_uploads`.
- Рендер и действия: `src/render/workspace.js`.
- Доступ v1 совместный: owner/editor/guest могут добавлять и редактировать заметки и ссылки внутри проекта.
- Заметки используют lazy-loaded Tiptap editor и сохраняют HTML в `note.body`; поддерживают rich-text, активную toolbar-панель, undo/redo, настоящие task-list чекбоксы, изображения по URL, typography-автоформатирование, горячие клавиши, режим просмотра перед редактированием, прикреплённые файлы до 10 MB и предпросмотр изображений в popup. Остальные типы файлов скачиваются.
- Разрешённые типы файлов: PDF, DOCX, XLSX, CSV, TXT, PNG, JPG/JPEG, WebP. Owner может удалить любой файл заметки, участник только свой.
- Обзор рабочей зоны сделан компактным: закреплённые ссылки в плотной сетке, последние заметки короткими строками; подробная работа остаётся в разделах `Заметки` и `Ссылки`.
- События логируются в журнал как `workspace_note_changed`, `workspace_link_changed`, `workspace_file_uploaded`, `workspace_file_deleted`; в журнале есть фильтр `Рабочая зона`.
