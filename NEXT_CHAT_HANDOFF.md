...
- Route: `/app/workspace`.
- Данные хранятся в существующем `workspace.state`: `S.workspaceArea = { notes, links }`.
...
- Журнал получает события `workspace_note_changed` и `workspace_link_changed`; в фильтрах журнала есть `Рабочая зона`.
