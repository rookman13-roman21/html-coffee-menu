...
Для быстрых архитектурных решений читать `ARCHITECTURE.md`: там домены, источники правды, матрица ролей, API командной работы, правила безопасности данных и smoke-сценарии.
...
На 22-23 июня 2026 обновлён раздел стартовых вложений и admin-библиотека:

- Клиентский бюджет получил сортировку категорий по порядку, сумме и количеству позиций, поиск по позициям, проценты/прогресс по категориям и кнопки развернуть/свернуть всё.
- Admin-библиотека `oc_library` теперь показывает все категории клиентского бюджета, включая пустые (`Стартовый склад`, `Брендинг`, `Оборотный резерв` и др.), а `+ Добавить` создаёт позицию в выбранной категории.
- В admin-библиотеке добавлены empty state, подсказка `Где используется`, блок полноты `Заполнено X из 12 разделов`, скрытие бессмысленного подфильтра `Все 0`.
- В admin `Пресеты` исправлена обрезка первой строки в Safari, добавлены `Скрыть уже добавленные`, бейдж `В пресете`, группировка текущего пресета по категориям бюджета.
- Общий экспорт `Excel (xlsx)` из верхнего меню исправлен: `src/ui/misc.js` импортирует `GROUP_LABEL`, устранён конфликт `mergeCells` в ExcelJS, добавлен общий `try/catch` и DOM-вставка ссылки перед `a.click()` для Safari. Проверенный production-коммит: `c1d1ef4 fix: handle full excel download errors`.

На 23 июня 2026 обновлён финансовый контур клиента:

- Во вкладке `План продаж` добавлен блок `Дополнительные продажи` для выпечки, десертов, еды и импульсных позиций. Поддерживаются модели `% чеков` и `шт/день`, пресеты, фильтр по категориям и подсказки в заголовках таблицы.
- `salesMetrics()` теперь считает общий план продаж: напитки + дополнительные позиции. Средний чек строится как `средний чек напитков + вклад доп. продаж`, а количество чеков в день автоматически равно количеству напитков/чеков.
- `Финмодель`, P&L, What-if, PDF и Excel используют общий план продаж, показывают вклад доп. продаж, общий food-cost, покрытие ТБУ, запас прочности и отклонение FC от целевого значения.
- Из полного PDF-отчёта убран блок `Рейтинг напитков`; Excel-отчёт сохраняет рейтинг на dashboard как рабочий аналитический блок.
- Мобильная версия обновлена: KPI финмодели компактно раскладываются по 2 в ряд, таблица доп. продаж сохраняет горизонтальный скролл и все desktop-колонки, итоговая строка таблицы напитков больше не обрезается.

На 24 июня 2026 доработан блок постоянных расходов в `Финмодель`:

- В модалке расхода `Фиксированная сумма ₽` переименована в `Сумма ₽/мес`, а галочка `Переменная` — в `Масштабировать в сценариях`, чтобы не путать базовую сумму с поведением в сценариях.
- Переключатель `% от выручки` теперь корректно показывает процентное поле: обработчики `_fceTypeChange`, `_fcePctHint`, `_fceShareToggle`, `_fceShareUpdate` экспортируются в `window`.
- Таблица постоянных расходов получила более оформленный контейнер, спокойный системный стиль для авто-ФОТ и summary-плашки `ручные`, `ФОТ`, `масштаб.`.
- На summary-плашки добавлены hover-подсказки через `data-tip`: что входит в ручные расходы, откуда берётся ФОТ и что значит масштабируемая сумма.

На 24 июня 2026 добавлен v1 слой командной работы для клиентов, которые запускают кофейню:

- Появились рабочие пространства `workspaces`: это общий проект кофейни поверх текущих локаций/точек.
- Старый `user_state` остаётся fallback-источником миграции: при первом входе пользователя создаётся личный workspace и в него переносится текущий state.
- `GET /api/state` и `PUT /api/state` теперь работают с активным workspace, а не только с `user_id`.
- В меню проекта добавлены `Команда проекта`, `Журнал действий` и переключение между проектами.
- Владелец проекта может создать invite-link по email; участник v1 получает роль `editor` и может редактировать общий проект.
- Право создавать свои проекты привязано к существующим клиентским доступам `access_drinks` / `access_finance`. Приглашённый без такого доступа входит как `guest`: редактирует приглашённый проект, но не создаёт свои проекты.
- Регистрация по invite-link сразу активирует guest-аккаунт и принимает приглашение, если email совпадает с приглашением.
- Журнал действий append-only: фиксирует вход/переключение проекта, приглашения, локации, бюджет открытия, финмодель/ФОТ, план продаж, рецепты, поставщиков, экспорты, полный сброс проекта владельцем и заблокированные критические попытки не-владельца. Autosave сам по себе не логируется; `project_opened` throttled на backend до одного события на пользователя/проект за 30 минут. В UI журнала есть фильтры по разделам, роль участника (`владелец` / `гость` / `платный`) и визуальная важность событий.

Safety baseline командной работы после production-теста guest-invite:

- `localStorage` считается только кэшем UI; если пользователь потерял доступ к workspace, frontend очищает локальные заведения и уходит в no-access, а не продолжает работать со старыми данными.
...
- Управление проектом вынесено в `/app/settings`: верхний dropdown оставлен быстрым переключателем проекта/заведения, а разделы `Аккаунт`, `Проект`, `Команда`, `Заведения`, `Журнал`, `Восстановление`, `Интеграции` живут на отдельной странице настроек.
...
- серверное состояние клиентского кабинета: `workspaces.state_json`;
- `user_state.state_json` остается fallback для миграции старых аккаунтов;
- браузерные ключи localStorage имеют scope `user + workspace_id`:
  - `mbs_locations__<user>__ws_<workspace_id>`;
  - `mbs_active_loc__<user>__ws_<workspace_id>`;
  - `mbs_loc_<id>__<user>__ws_<workspace_id>`.

Не возвращать глобальные ключи `mbs_locations`, `mbs_active_loc`, `mbs_loc_*` без user scope и не откатываться к keys только по user без `workspace_id`: иначе заведения разных проектов смешиваются.
...
- `HTML_coffee_menu/src/render/sales.js` — план продаж, таблица напитков и блок дополнительных продаж.
- `HTML_coffee_menu/src/render/finmodel.js` — финансовая модель, P&L, прогноз, KPI и сценарии.
- `HTML_coffee_menu/src/utils/calc.js` — расчёты бюджета, продаж, доп. продаж, ТБУ и финмодели.
- `HTML_coffee_menu/src/ui/misc.js` — PDF/Excel-экспорт, подсказки, печать и вспомогательные UI-действия.
- `HTML_coffee_menu/src/ui/updaters.js` — обработчики изменения плана продаж, доп. продаж, бюджета и финмодели.
...
389ec80 feat: add account settings panel
71624de feat: show avatars in activity log
882affc fix: align activity avatar to the right
```
...
## 19. Текущий продуктовый слой: рабочая зона проекта

- Вкладка `Рабочая зона` стоит первой в верхней навигации перед `Бюджет`.
- Route: `/app/workspace`.
- Данные заметок и ссылок хранятся в существующем `workspace.state`: `S.workspaceArea = { notes, links }`; приватные файлы заметок хранятся отдельно в backend-таблице `workspace_files` и runtime-папке `server/data/workspace_uploads`.
- Рендер: `src/render/workspace.js`.
- В v1 owner/editor/guest могут добавлять и редактировать заметки и полезные ссылки в общем проекте.
- Заметки используют Tiptap (`@tiptap/core`, `starter-kit`, `link`, `placeholder`, `task-list`, `task-item`, `image`, `typography`) как lazy-loaded rich-text редактор: модуль загружается только при открытии редактора заметки. Существующий формат хранения остаётся HTML в `note.body`.
- Заметки поддерживают активную toolbar-панель, undo/redo, настоящие task-list чекбоксы, изображения по URL, typography-автоформатирование, горячие клавиши, типы заметок, закрепление, отдельный режим просмотра перед редактированием и файлы до 10 MB: PDF, DOCX, XLSX, CSV, TXT, PNG, JPG/JPEG, WebP. Владелец может удалить любой файл заметки, участник — только свой.
- Визуально редактор должен выглядеть как рабочий документ: document-area вместо простого поля ввода, быстрые действия `Чеклист / Заголовок / Ссылка / Файл`, компактные вложения и dark/mobile стили.
- При клике на изображение в файлах заметки открывается предпросмотр в модальном окне; остальные файлы скачиваются. Для macOS/Safari есть fallback определения типа по расширению файла.
- Обзор рабочей зоны сделан компактным: закреплённые ссылки показываются плотной сеткой, последние заметки — короткими строками; полные карточки остаются в разделах `Заметки` и `Ссылки`.
- Журнал получает события `workspace_note_changed`, `workspace_link_changed`, `workspace_file_uploaded`, `workspace_file_deleted`; в фильтрах журнала есть `Рабочая зона`.

## 20. Что специально не делать без запроса
